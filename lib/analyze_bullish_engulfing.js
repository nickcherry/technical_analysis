...
module.exports = (sortedCandles, { lookbehindCandles, lookaheadCandles, groupSizeForPctPriceIncreaseProbability }) => {
  ...
  for (let i = lookbehindCandles; i < sortedCandles.length; i++) {
    ...
    const isBullishEngulfing = (
      ...
      recentCandles[recentCandles.length - 1].low > candle.open // Recent short-term trend must be bearish
    );
  ...
  const overallMinPctPriceChange = roundToInc((Math.min(...events.map((e) => e.minPctPriceChange))), groupSizeForPctPriceIncreaseProbability);
  ...
  let seenDifferingProbabilities = false;
  for (i = overallMinPctPriceChange; i <= overallMaxPctPriceChange; i += groupSizeForPctPriceIncreaseProbability) {
    ...
    // We don't need to include data points where both the probability and control probability are 1
    if (!seenDifferingProbabilities && probability === controlProbability && probability === 1) continue;
    seenDifferingProbabilities = true;
