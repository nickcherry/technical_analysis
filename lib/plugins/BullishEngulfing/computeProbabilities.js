...
//
// Calculates the probabilities of a candle increasing by various percent ranges
// given an array of sorted candles and an options object, which should have the following properties:
...
module.exports = (sortedCandles, { lookbackCandles, lookaheadCandles, groupSizeForPctPriceIncreaseProbability, allowedWickToBodyRatio }) => {
  ...
  for (let i = lookbackCandles; i < sortedCandles.length; i++) {
    const candle = getCandleWithStats(sortedCandles, i, { lookbackCandles, allowedWickToBodyRatio });
    ...
    unfilteredEvents.push({
      ...
      maxPctPriceChange,
      minPctPriceChange,
      pctVolumeChange: candle.pctVolumeChange,
      isBullishEngulfing: candle.isBullishEngulfing,
    });
  }
